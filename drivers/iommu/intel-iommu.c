...
static inline bool nested_mode_support(void)
{
	...
	for_each_active_iommu(iommu, drhd) {
		...
		if (!sm_supported(iommu) || !ecap_nest(iommu->ecap) ||
		    ((iommu->cap & VTD_CAP_MASK) !=
					(prev->cap & VTD_CAP_MASK)) ||
		    ((iommu->ecap & VTD_ECAP_MASK) !=
					(prev->ecap & VTD_ECAP_MASK))) {
			ret = false;
			break;
		}
	}
			    ...
			    enum iommu_attr attr, void *data)
{
	...
	case DOMAIN_ATTR_IOASID_SID:
		...
		if ((dmar_domain->ioasid_sid != INVALID_IOASID_SET) &&
			...
			pr_warn_ratelimited("Tries to set multiple ioasid_set"
					    " (%d:%d) to a domain\n",
					...
					void *data)
{
	...
	info = (struct iommu_nesting_info *) data;
				       ...
				       enum iommu_attr attr, void *data)
{
	...
	case IOMMU_DOMAIN_UNMANAGED:
		...
		case DOMAIN_ATTR_NESTING_INFO:
			return intel_iommu_get_nesting_info(domain, data);
		default:
			return -ENODEV;
		}
		break;
